...
import { ArrowLeft, Check, CheckCircle2, FileSpreadsheet, Loader2, Search, Upload } from "lucide-react"
...
type Unregistered = { id:string; display_name:string; normalized_name:string }
...
export function ArbiterFeeImportWorkflow() {
  ...
  const unresolved = newItems.filter((item) => !item.resolution_confirmed || (!item.member_id&&!item.unregistered_referee_id))
  ...
  async function resolve(item: Item, target: string, saveAlias: boolean) {
    ...
    try {
      const [targetType,targetId]=target.split(":",2) as ["member"|"unregistered",string]
      if (saveAlias) {
        const aliasResponse = await fetch("/api/admin/finance/arbiter-fees/mappings", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ arbiter_name:item.arbiter_referee_name, member_id:targetId }) })
      ...
      const body=targetType==="member"?{target_type:"member",member_id:targetId}:{target_type:"unregistered",unregistered_referee_id:targetId}
      const response = await fetch(`/api/admin/finance/arbiter-fees/imports/${data?.batch.id}/items/${item.id}`, { method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) })
      const payload = await response.json().catch(()=>null)
  ...
  async function createUnregistered(item:Item){if(busy||!data)return;const displayName=window.prompt("Display name for the pending financial account:",item.arbiter_referee_name)?.trim();if(!displayName)return;setBusy(true);try{const response=await fetch("/api/admin/finance/unregistered",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({display_name:displayName,arbiter_name:item.arbiter_referee_name})});const payload=await response.json().catch(()=>null);if(!response.ok)throw new Error(payload?.error??"Unable to create the pending financial account.");const target=payload.unregistered as Unregistered;const resolution=await fetch(`/api/admin/finance/arbiter-fees/imports/${data.batch.id}/items/${item.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_type:"unregistered",unregistered_referee_id:target.id})});const resolved=await resolution.json().catch(()=>null);if(!resolution.ok)throw new Error(resolved?.error??"Unable to resolve the referee.");setData(resolved);toast.success("Pending financial account created and selected.")}catch(error){toast.error(error instanceof Error?error.message:"Unable to create the pending financial account.")}finally{setBusy(false)}}
  ...
  return <div className="space-y-6">
    ...
    {!data && <form onSubmit={upload} className="overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#0B1513] to-[#070B0A]">
      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.75fr)] lg:items-center"><div><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-yellow-400/20 bg-yellow-400/10"><FileSpreadsheet className="h-5 w-5 text-yellow-300"/></div><h2 className="mt-4 text-xl font-semibold text-white">Upload Arbiter schedule</h2><p className="mt-2 max-w-xl text-sm leading-6 text-gray-400">Select the weekly fee-eligible export. We parse the first worksheet and build a server-authoritative preview before anything can be posted.</p><div className="mt-4 flex flex-wrap gap-2 text-xs text-gray-400"><span className="rounded-full border border-white/10 px-3 py-1">.xls / .xlsx</span><span className="rounded-full border border-white/10 px-3 py-1">10 MB max</span><span className="rounded-full border border-white/10 px-3 py-1">10,000 rows max</span></div></div>
      <div className="rounded-xl border border-dashed border-white/15 bg-black/20 p-4"><label className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Arbiter export</label><Input className="mt-3" name="file" type="file" accept=".xls,.xlsx" required disabled={busy} onChange={(event)=>setSelectedFilename(event.target.files?.[0]?.name??"")}/><p className="mt-2 min-h-5 truncate text-xs text-gray-500">{selectedFilename||"No file selected"}</p><Button className="mt-4 w-full bg-yellow-400 text-black hover:bg-yellow-300" disabled={busy}>{busy?<Loader2 className="animate-spin"/>:<Upload/>}{busy?"Parsing...":"Parse and Preview"}</Button></div></div>
    </form>}
    {data && <>
      <Summary data={data}/>
      {success ? <Success result={success} batch={data.batch}/> : <>
        ...
        <div className="space-y-3">{refereeGroups.map((items)=><RefereeCard key={items[0].normalized_referee_name} items={items} members={data.members} unregistered={data.unregistered} memberMap={memberMap} busy={busy} onResolve={resolve} onCreateUnregistered={createUnregistered}/>)}</div>
...
function RefereeCard({items,members,unregistered,memberMap,busy,onResolve,onCreateUnregistered}:{items:Item[];members:Member[];unregistered:Unregistered[];memberMap:Map<string,Member>;busy:boolean;onResolve:(item:Item,target:string,save:boolean)=>void;onCreateUnregistered:(item:Item)=>void}) { const item=items[0]; const [choice,setChoice]=useState(item.member_id?`member:${item.member_id}`:item.unregistered_referee_id?`unregistered:${item.unregistered_referee_id}`:""); const [save,setSave]=useState(false); const fresh=items.filter(i=>i.item_status==="new"); const center=fresh.filter(i=>i.role==="center").length; const ar=fresh.length-center; const imported=items.filter(i=>i.item_status==="already_imported").length; const centerFee=fresh.filter(i=>i.role==="center").reduce((n,i)=>n+Number(i.fee_cents),0); const arFee=fresh.filter(i=>i.role!=="center").reduce((n,i)=>n+Number(i.fee_cents),0); const member=item.member_id?memberMap.get(item.member_id):null; const provisional=item.unregistered_referee_id?unregistered.find(u=>u.id===item.unregistered_referee_id):null; const resolved=fresh.length===0||fresh.every(i=>i.resolution_confirmed&&(i.member_id||i.unregistered_referee_id)); return <article className="rounded-2xl border border-white/10 bg-[#0B0F0F]/70 p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-white">{item.arbiter_referee_name}</h3><span className={`rounded-full px-2 py-0.5 text-xs ${resolved?"bg-emerald-400/10 text-emerald-300":"bg-amber-400/10 text-amber-300"}`}>{resolved?"resolved":item.initial_match_state}</span></div><p className="mt-1 text-sm text-gray-400">{member?`${member.full_name} · ${member.status}`:provisional?`${provisional.display_name} · Pending financial account`:"No confirmed financial identity"}</p><p className="mt-2 text-xs text-gray-500">{center} Center · {ar} AR · {fresh.length} new · {imported} already imported</p><p className="mt-1 text-xs text-gray-500">Gross {formatUsdFromCents(fresh.reduce((n,i)=>n+Number(i.gross_earnings_cents),0))} · Center fee {formatUsdFromCents(centerFee)} · AR fee {formatUsdFromCents(arFee)} · Total fee {formatUsdFromCents(centerFee+arFee)}</p></div>{!resolved&&<div className="w-full space-y-3 lg:max-w-md"><select value={choice} onChange={(e)=>{setChoice(e.target.value);if(!e.target.value.startsWith("member:"))setSave(false)}} className="h-10 w-full rounded-md border border-white/10 bg-[#07100E] px-3 text-sm text-white"><option value="">Leave unresolved</option><optgroup label="CAFLA members">{members.map(m=><option key={m.id} value={`member:${m.id}`}>{m.full_name} — {m.email} ({m.status})</option>)}</optgroup><optgroup label="Pending financial accounts">{unregistered.map(u=><option key={u.id} value={`unregistered:${u.id}`}>{u.display_name}</option>)}</optgroup></select>{choice.startsWith("member:")&&<label className="group flex cursor-pointer items-center gap-2.5 rounded-lg px-1 py-1 text-xs leading-5 text-gray-400 transition-colors hover:text-gray-300"><span className="relative flex h-[18px] w-[18px] shrink-0 items-center justify-center"><input type="checkbox" checked={save} onChange={(e)=>setSave(e.target.checked)} className="peer h-[18px] w-[18px] cursor-pointer appearance-none rounded-[5px] border border-emerald-900/70 bg-[#07100E] transition-colors hover:border-yellow-400/60 checked:border-yellow-400 checked:bg-yellow-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B0F0F]"/><Check aria-hidden="true" className="pointer-events-none absolute h-3.5 w-3.5 stroke-[3] text-[#07100E] opacity-0 transition-opacity peer-checked:opacity-100"/></span><span>Save this Arbiter name mapping for future imports</span></label>}<div className="flex flex-wrap gap-2"><Button size="sm" disabled={busy||!choice} onClick={()=>onResolve(item,choice,save)} className="bg-yellow-400 text-black hover:bg-yellow-300">Confirm Selection</Button><Button size="sm" variant="outline" disabled={busy} onClick={()=>onCreateUnregistered(item)} className="border-emerald-900/60 bg-[#07100E] text-white hover:border-yellow-400/50 hover:text-yellow-300">Track as New Unregistered</Button></div></div>}</div></article> }
...
function WorkflowProgress({step}:{step:1|2|3}) { return <ol className="grid grid-cols-3 overflow-hidden rounded-2xl border border-white/10 bg-black/20">{["Upload","Review & Resolve","Confirm"].map((label,index)=>{const number=index+1;const active=number===step;const complete=number<step;return <li key={label} className={`flex min-w-0 items-center gap-2 border-r border-white/10 px-3 py-3 last:border-r-0 sm:px-5 ${active?"bg-yellow-400/[0.08]":""}`}><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${complete?"bg-emerald-400 text-black":active?"bg-yellow-400 text-black":"bg-white/10 text-gray-400"}`}>{complete?"✓":number}</span><span className={`truncate text-xs font-medium sm:text-sm ${active?"text-yellow-200":complete?"text-emerald-300":"text-gray-500"}`}>{label}</span></li>})}</ol> }
