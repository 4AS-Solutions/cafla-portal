...
export function RecordTransactionDialog({
  ...
  members?: AdminFinanceMember[];
  preselectedMember?: AdminFinanceMember;
}) {
  ...
  function changeType(next: BoardTransactionType) {
    setType(next);
    ...
    try {
      const response = await fetch("/api/admin/finance/transactions", {
        ...
        body: JSON.stringify({
          ...
          payment_method:
            type === "payment" ? form.get("payment_method") : null,
          adjustment_direction:
            type === "adjustment" ? form.get("adjustment_direction") : null,
          idempotency_key: idempotencyKey,
        }),
      });
      const payload = await response.json().catch(() => null);
      if (!response.ok)
        throw new Error(payload?.error ?? "Unable to record the transaction.");
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto border-white/10 bg-[#07100E] text-white sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new immutable Finance ledger entry.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-5">
          {preselectedMember ? (
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
              <p className="font-medium text-white">
                {preselectedMember.full_name}
              </p>
              <p className="text-xs text-gray-500">
                {preselectedMember.email} · {preselectedMember.status}
              </p>
            </div>
          ) : (
            <Field label="Member">
              ...
              >
                ...
                {members?.map((member) => (
                  <option key={member.id} value={member.id}>
                    {member.full_name} — {member.email} ({member.status})
                  </option>
                ))}
              </select>
            </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Transaction type">
              <select
                value={type}
                onChange={(event) =>
                  changeType(event.target.value as BoardTransactionType)
                }
                className={selectClass}
              >
            ...
            {type === "adjustment" && (
              <Field label="Adjustment direction">
                ...
                >
                  ...
                  <option value="member_owes_cafla">Member owes CAFLA</option>
                  <option value="cafla_owes_member">CAFLA owes member</option>
                </select>
              </Field>
            )}
          </div>
          <Field label="Description" hint="Visible to the member">
            <Input
              value={description}
          ...
          <Field label="Internal note" hint="Board only">
            <Textarea
              name="internal_notes"
              maxLength={2000}
          ...
          <DialogFooter>
            <Button
              type="button"
              variant="ghost"
              className="border border-white/10 bg-white/[0.03] text-gray-300 hover:bg-white/[0.06] hover:text-white focus-visible:ring-yellow-400/40"
              disabled={pending}
...
const selectClass =
  "flex h-9 w-full rounded-md border border-input bg-[#07100E] px-2.5 text-sm text-white outline-none focus:border-yellow-400/50";
