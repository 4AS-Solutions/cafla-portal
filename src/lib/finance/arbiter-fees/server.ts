...
import { findPendingUnregisteredAlias, prepareArbiterFeeItems, normalizedRefereeName } from "./workflow"
...
export async function createArbiterFeeImport(file: File, actorId: string) {
  ...
  const [memberRows, aliasRows, postedRows, unregisteredAliases, linkedRows] = await Promise.all([
    ...
    allRows<{ normalized_arbiter_name:string; unregistered_referee_id:string }>((from,to)=>db.schema("finance").from("unregistered_arbiter_aliases").select("normalized_arbiter_name,unregistered_referee_id").order("id").range(from,to)),
    allRows<{ unregistered_referee_id:string }>((from,to)=>db.schema("finance").from("unregistered_referee_links").select("unregistered_referee_id").order("id").range(from,to)),
  ])
  ...
  const linkedIds = new Set(linkedRows.map((row)=>row.unregistered_referee_id))
  const provisionalAliases=unregisteredAliases.map(row=>({normalizedArbiterName:row.normalized_arbiter_name,unregisteredRefereeId:row.unregistered_referee_id}))
  ...
  if (items.length) {
    const rows = items.map((item) => { const provisionalId=findPendingUnregisteredAlias(item.arbiterRefereeName,provisionalAliases,linkedIds); return ({
      ...
      resolution_confirmed: item.itemStatus === "new" && (Boolean(provisionalId) || item.state === "exact"),
      resolution_method: provisionalId ? "unregistered_alias" : item.state === "exact" ? (item.reason === "trusted_alias" ? "trusted_alias" : "exact_full_name") : "pending",
...
export async function getArbiterFeeImport(importId: string) {
  const db = getSupabaseAdmin()
  const [{ data: batch, error: batchError }, { data: items, error: itemsError }, members, unregistered, links] = await Promise.all([
    ...
    allRows<{id:string;display_name:string;normalized_name:string}>((from,to)=>db.schema("finance").from("unregistered_referees").select("id,display_name,normalized_name").order("display_name").range(from,to)),
    allRows<{unregistered_referee_id:string}>((from,to)=>db.schema("finance").from("unregistered_referee_links").select("unregistered_referee_id").range(from,to)),
  ])
  if (batchError) throw batchError
  if (itemsError) throw itemsError
  const linked=new Set(links.map((row)=>row.unregistered_referee_id))
  return { batch, items: items ?? [], members, unregistered:unregistered.filter((row)=>!linked.has(row.id)) }
}
