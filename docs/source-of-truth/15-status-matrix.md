...
| Reports | `development.referee_report_detail` and `development.referee_report_score` | Database calculation views | CURRENT with TRANSITIONAL population | `05-match-reports.md`; active Dashboard/Development callers | Canonical participation population | Any Report row satisfies completion; same-date submission supplies timeliness; approval is not required. |
...
| Quiz | Quiz V2 | Application + `development` database | CURRENT | Current Portal/Admin Quiz runtime and repository DB baseline | — | Public Quiz V1 is RETIRED. |
| Evaluations | `public.evaluations` + Development evaluation detail/score | Application + database | CURRENT | Current evaluation runtime and restored V2 chain | — | `public.evaluations` is not legacy. |
| Development | Development V2 metric aggregation | Application + `development` database | CURRENT | `/portal/development`, metric query helpers and repository DB baseline | — | Population rules are currently distributed. |
...
| Documentation | Source of Truth suite | Documentation | TRANSITIONAL | `docs/source-of-truth/` | Complete evidence-based module passes | Foundation and modules 01–06, 12, and 12a are substantive; remaining reviews continue. |
...
- Review scope: foundation, System Overview, Auth/Authorization, Members,
  Development participation, Matches, Match Reports, Attendance,
